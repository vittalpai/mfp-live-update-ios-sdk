...
public class ConfigurationServiceInstance: ConfigurationService {
    private let serviceURL: String = "adapters/configService"
    
    public static let sharedInstance = ConfigurationServiceInstance()
    
    private init() {
    }
    
    // Get configuration by segment
    public func getConfig (segment: String, useCache: Bool = true, completionHandler: (configuration: Configuration?, error: NSError?) -> Void) {
    ...
    public func getConfig (params: [String:String], useCache: Bool = true, completionHandler: (configuration: Configuration?, error: NSError?) -> Void) {
        let url = NSURL(string: serviceURL)!
